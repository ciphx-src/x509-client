...
use crate::provided::openssl::iter::OpenSSLX509Iterator;
use std::path::Path;
use tokio::fs::File;
...
async fn test_cer() {
    ...
    let iter = OpenSSLX509Iterator::from_cer(&certificate)
        .unwrap()
        .into_iter();
...
async fn test_pem() {
    ...
    let iter = OpenSSLX509Iterator::from_pem(&certificate)
        .unwrap()
        .into_iter();
    assert_eq!(1, iter.len());
}
...
async fn test_pkcs7() {
    ...
    let iter = OpenSSLX509Iterator::from_pkcs7(&certificate)
        .unwrap()
        .into_iter();
