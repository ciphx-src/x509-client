...
pub trait X509Iterator<I = Self>: IntoIterator
where
    Self: Sized,
{
    /// Error type
    ...
    fn from_cer<T: AsRef<[u8]>>(src: T) -> Result<I, Self::X509IteratorError>;
    /// Attempt to deserialize, assume input is a stack of zero or more PEM-encoded certificates
    fn from_pem<T: AsRef<[u8]>>(src: T) -> Result<I, Self::X509IteratorError>;
    /// Attempt to deserialize, assume input is a DER-encoded PKCS7 certificate bundle
    fn from_pkcs7<T: AsRef<[u8]>>(src: T) -> Result<I, Self::X509IteratorError>;
}
